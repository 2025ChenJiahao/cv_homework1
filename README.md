...
1. **任务一：LoG 滤波与边缘检测**
   ...
   - 有灰度图我们不考虑.mat文件（既然我们有 **lena512.bmp** 文件，这是一个标准的图像格式，我们可以直接用 OpenCV 来加载它作为灰度图，这比加载 .mat 文件更简单，并且可以移除对 scipy 库的依赖。）
...
从 “算法实现 - 参数分析 - 量化评估” 考察基础图像处理算法的理解与编程能力，核心目标是掌握经典边缘检测算法的原理、应用与评估方法。
试验要求写报告并且只要代码一份，我们可以把代码整合在一起：
